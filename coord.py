...
datetime.datetime.strptime(worknet['지원마감일'][0],'%y-%M-%d').strftime('%Y-%M-%d')
last_d = list(map(lambda x : x.replace('채용시까지  ',''),worknet['지원마감일']))
worknet['지원마감일']= last_d
last_d1 = list(map(lambda x : datetime.datetime.strptime(x,'%y-%M-%d').strftime('%Y-%M-%d'),worknet['지원마감일']))
first_d1 = list(map(lambda x : datetime.datetime.strptime(x,'%y-%M-%d').strftime('%Y-%M-%d'),worknet['지원시작일']))

worknet['지원마감일'] = last_d1
worknet['지원시작일'] = first_d1
# worknet.to_csv('C:/SEONWOO/enterprise/worknet_df1.csv') 윈도우 기반 

worknet = pd.read_csv('/Users/snoo/enterprise/worknet_df1.csv')
